...
- Tenant RBAC + Skill Entitlement Pack for local tenant/user scopes, allowed and denied skill policies, MCP-safe tool subsets, entitlement coverage drift review, proposed policy change preview, enforced denied invocation audit events, dashboard review, and ignored `data/entitlement_packs/` artifacts.
...
- Platform Operations Drill Pack for operator state observation across sandbox policy, run transparency, repository automation dry-run, worker scale-out, policy replay, bounded action loops, step verification, and ignored `data/platform_operations/` artifacts.
- Runtime Skill Quarantine Pack for SLO, reliability, prompt-governance, provider, owner-review, MCP exposure, kill-switch, rollback, and ignored `data/quarantine_packs/` artifacts.
...
- Worker Run Replay Pack for deterministic checkpoint replay across worker queue admission, sandbox preflight, invocation output, timeline stages, drift flags, and ignored `data/worker_replays/` artifacts.
...
- API Contract Snapshot + MCP Reviewer Collection + Tool Contract Drift Pack + Contract Remediation Run for fresh-clone verification of OpenAPI route count, `X-API-Key` protection, docs/api coverage, dashboard smoke alignment, generated artifact endpoints, demo flow endpoints, MCP tools/resources/prompts, manifest/MCP schema fingerprints, version drift remediation, bounded step verification, sample commands, and ignored `data/api_contracts/` Markdown/JSON artifacts.
...
`POST /tenants/entitlements/evaluate` returns per-skill allow/deny decisions, missing scopes, matched policies, denied skill ids, and `mcp_safe_tool_names`. `GET /tenants/entitlements/coverage` compares promoted MCP tools against tenant exact and wildcard policies, flags review-required wildcard rows, and includes denied-entitlement audit evidence. `GET /tenants/entitlements/access-review` adds privileged-policy rows, wildcard exposure, denied-audit pressure, break-glass drill checks, and bounded reviewer steps. `POST /tenants/entitlements/change-preview` overlays one proposed policy row without mutating live policy, then returns before/after matrices, MCP-safe tool delta, blast radius, and guardrail checks; `POST /tenants/entitlements/change-pack` writes `tenant_entitlement_change_preview_latest.json` and `.md`. Enforced denied invocations return `403`, create failed invocation history rows, and record `entitlement.denied` audit events. `POST /tenants/entitlements/pack` writes `tenant_entitlement_pack_latest.json` and `.md`; `POST /tenants/entitlements/review-pack` writes `tenant_entitlement_review_pack_latest.json` and `.md`; `POST /tenants/entitlements/access-review-pack` writes `tenant_entitlement_access_review_latest.json` and `.md` under ignored `data/entitlement_packs/`. Break-glass scopes are review evidence only and do not bypass normal entitlement enforcement.
...
Prepare and optionally apply a local kill switch for risky MCP skills:
...
Invoke-RestMethod http://localhost:8000/quarantine/apply `
  -Method POST `
  -Headers $headers `
  -ContentType "application/json" `
  -Body '{"skill_ids":["search_knowledge_base"],"actor":"platform-sre","reason":"SLO kill-switch drill"}'
...
`GET /quarantine/report` combines SLO release gates, reliability recommendations, prompt-governance findings, provider posture, ownership metadata, and MCP exposure into per-skill allow/monitor/quarantine decisions. `POST /quarantine/pack` writes `skill_quarantine_pack_latest.json` and `.md` under ignored `data/quarantine_packs/`. `POST /quarantine/apply` is explicit and disables only requested or recommended skills in the local registry, recording audit trace IDs for rollback review. The Streamlit dashboard has a `Skill Quarantine` view, and `python -m app.demo` prints quarantine readiness plus the artifact path.
...
Invoke-RestMethod http://localhost:8000/workers/replay-report -Method POST -Headers $headers -ContentType "application/json" -Body '{"max_replays":1}'
Invoke-RestMethod http://localhost:8000/workers/replay-pack -Method POST -Headers $headers -ContentType "application/json" -Body '{"max_replays":1}'
...
Get-ChildItem -Recurse -File data\worker_replays -ErrorAction SilentlyContinue | Select-Object FullName,Length,LastWriteTime
...
`POST /workers/replay-report` creates bounded replay runs for recent or selected non-replay worker records and compares status, output, queue decision, sandbox decision, and timeline stages. `POST /workers/replay-pack` writes `worker_run_replay_pack_latest.json` and `.md` under ignored `data/worker_replays/`. The Streamlit dashboard has a `Worker Replay` view, and `python -m app.demo` prints replay readiness, drift count, and artifact path.
