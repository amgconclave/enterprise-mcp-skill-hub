...
- Skill Marketplace tests cover `GET /marketplace/catalog`, `GET /marketplace/promotion-gate/{skill_id}`, `POST /marketplace/rollout-pack`, deterministic Tenant Rollout scenarios, approval-backed promotion gates, blocked and review-required rollouts, disabled-skill rollout/invocation blocks, usage/version/MCP exposure signals, artifact contents under `data/marketplace_packs/`, dashboard smoke wiring, Artifact Inventory, and API Contract coverage.
...
- Runtime Skill Quarantine tests cover `GET /quarantine/report`, `POST /quarantine/pack`, `POST /quarantine/apply`, SLO/reliability/prompt/provider kill-switch evidence, human-review queue rows, local registry disable behavior, generated artifacts under `data/quarantine_packs/`, dashboard smoke wiring, Artifact Inventory, and API Contract coverage.
- Skill Ownership tests cover `GET /ownership/matrix`, `POST /ownership/pack`, manifest owner/team/channel metadata, support tiers, risk-aware escalation routes, handoff plans, generated artifacts under `data/ownership_packs/`, dashboard smoke wiring, Artifact Inventory, API Contract coverage, and demo output.
...
- Policy Replay tests cover `GET /policy/replay-drift`, `POST /policy/replay-pack`, fresh-clone baseline allow/deny scenarios, historical policy decision replay, missing-evidence HITL queue rows, bounded review steps, generated artifacts under `data/policy_replay/`, dashboard smoke wiring, Artifact Inventory, API Contract coverage, and demo output.
...
- Supply Chain tests cover `GET /supply-chain/report`, `POST /supply-chain/pack`, direct dependency SBOM rows, manifest hashes, local license policy, pinning signals, optional external-provider dependency gates, generated artifacts under `data/supply_chain/`, dashboard smoke wiring, Artifact Inventory, and API Contract coverage.
...
- API Contract tests cover `GET /api/contract-audit`, `POST /api/reviewer-collection`, `POST /api/contract-drift-pack`, `GET /api/contract-remediation-run`, `POST /api/contract-remediation-pack`, OpenAPI route count, auth-protected endpoint count, docs/api coverage, dashboard smoke alignment, generated artifact endpoint coverage, demo flow endpoint coverage, MCP tools/resources/prompts coverage, generated artifacts under `data/api_contracts/`, bounded remediation step verification, expected status codes, sample commands, and local-only limitations.
...
- The demo prints Supply Chain readiness and the exported Supply Chain Pack path.
...
- The demo prints Policy Replay readiness, drift count, and exported replay pack path.
