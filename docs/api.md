...
- `POST /marketplace/approval-pack` - writes the Skill Marketplace Approval Workflow Pack Markdown/JSON under ignored local folder `data/marketplace_packs/`.
...
- `GET /lineage/report` - returns skill lineage records with manifest/schema hashes, MCP exposure, prompt/resource/workflow links, policy controls, graph nodes/edges, and reviewer actions.
- `POST /lineage/pack` - writes the Skill Lineage Pack Markdown/JSON under ignored local folder `data/lineage/`.
...
- `GET /api/contract-remediation-run` - returns a read-only contract remediation run with state observation, bounded action-loop steps, step verification commands, remediation backlog, and task-sandbox limitations.
- `POST /api/contract-remediation-pack` - writes the Contract Remediation Run Pack Markdown and JSON under ignored local folder `data/api_contracts/`.
...
Invoke-RestMethod "http://localhost:8000/marketplace/approvals/$($approval.approval_id)/stage" `
  -Headers $headers `
  -Method POST `
  -ContentType "application/json" `
  -Body '{"actor":"release-manager","next_stage":"tenant_canary"}'

Invoke-RestMethod http://localhost:8000/marketplace/approval-pack `
  -Headers $headers `
  -Method POST
```
...
Invoke-RestMethod http://localhost:8000/api/contract-remediation-pack `
  -Method POST `
  -Headers $headers `
  -ContentType "application/json" `
  -Body '{"actor":"contract-remediation-reviewer"}'
```
...
The Contract Remediation Run keeps remediation local and read-only. `GET /api/contract-remediation-run` observes the current audit/drift state and returns a bounded sequence of verification steps for route/auth coverage, manifest-to-MCP fingerprints, FastAPI governance fields, docs/dashboard/artifact wiring, local evidence export, and release handoff. `POST /api/contract-remediation-pack` writes `contract_remediation_pack_latest.json` and `contract_remediation_pack_latest.md` with state observation, run transparency, step verification, task-sandbox limitations, and any remaining remediation backlog.
